...
with sqlite3.connect("arugamObmen.db") as arugamDB:  #база данных по обмену
    sql = arugamDB.cursor()
...
with sqlite3.connect("arugamObmen.db") as arugamDB:
    sql = arugamDB.cursor()
    table = """CREATE TABLE IF NOT EXISTS `obmenniki` (
    loginTelegram TEXT,
    idTelegram TEXT,
    location TEXT,
    MoneyCount FLOAT NOT NULL DEFAULT 0
    )"""
    sql.executescript(table)
...
def addSQL(parametr, num, id):  # обновляет заданный параметр в sql по одному
    try:
        with sqlite3.connect("arugamObmen.db") as arugamDB:
            sql = arugamDB.cursor()
            sql.execute(f"""UPDATE obmen SET {parametr} = {num} WHERE id = {id}""")
            arugamDB.commit()
    except sqlite3.Error as error:
        print("Ошибка при работе с SQLite", error)

def ChangeCountMoney(user_id, count, todo): #меняет колво денег у менялы
    try:
        with sqlite3.connect("arugamObmen.db") as arugamDB:
            sql = arugamDB.cursor()
            if todo == "-":
                sql.execute(f"""UPDATE obmenniki set MoneyCount = 
                (MoneyCount - {int(count)}) where idTelegram = {user_id}""")
            elif todo == "+":
                sql.execute(f"""UPDATE obmenniki set MoneyCount = 
                (MoneyCount + {int(count)}) where idTelegram = {user_id}""")
            arugamDB.commit()
...
def finishOperation():  #кнопка закрытия текущей сделки
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(f'Завершить заполнение', callback_data="finish")]
        ]
    )

@dispatcher.callback_query_handler(lambda c: c.data == "finish", state="*")  #закрывает текущую сделку
async def finish(message: types.Message, state: FSMContext):
    addSQL("`statusDone`", "`done`", countNum)
    await state.finish()
    await message.answer("Сделка закрыта и удалена из открытых")
...
async def CourseChoise(message: types.Message, state: FSMContext):
    course = message.text
    addSQL("`costOfUSDT`", message.text, countNum)
    await state.update_data(Course=course)
    await message.answer(f"Сохранен курс {course} lkr за 1 USDT", reply_markup=finishOperation())
...
async def begin(message: types.Message):
    if message.chat.id == id_egor:# or message.chat.id == id_gosha:
        ...
        await message.answer(f"Если отправить просто цифру, откроет сделку с соответствующим id.\n"
                             f"Для закрытия сделки, отправь информацию в формате: 42 @Primer 420 usdt 155400 lkr")

@dispatcher.message_handler(content_types="text")  # главная функция, обрабатывающая текстовые сообщения
    ...
    if message.chat.id == id_egor:# or message.chat.id == id_gosha:
        ...
        if len(text) == 1 and text[0].isdigit():  # если пришла цифра, открывает сделку с таким номером
            try:
                ...
                print(error)
        elif text[0].isdigit():  # 42 @example 420 usdt 155400 lkr
            try:
                addSQL("`summUSDT`", text[2], text[0])
                ChangeCountMoney(message.chat.id, text[4], "-")
                ...
                await bot.send_message(message.chat.id, "Неверный формат ввода: 42 @Primer 420 usdt 155400 lkr")
